...
int RobotS ::getDistance() const {
    return abs(sherlock->getCurrentPosition().getRow() - init_pos.getRow()) +
           abs(sherlock->getCurrentPosition().getCol() - init_pos.getCol());
}
...
int RobotSW ::getDistance() const {
    int r = this->pos.getRow(),
        c = this->pos.getCol(),
        rS = this->sherlock->getCurrentPosition().getRow(),
        cS = this->sherlock->getCurrentPosition().getRow(),
        rW = this->watson->getCurrentPosition().getRow(),
        cW = this->watson->getCurrentPosition().getRow();
    return abs(r - rS) + abs(c - cS) + abs(r - rW) + abs(c - cW);
...
if (!canUse(obj, robot)) return;
    ...
    switch (robot->getRobotType()) {
        case C:
            if (sherlock->GetHp() > 500) sherlock->setPosition(robot->getNextPosition());
...
void PassingCard ::use(Character* obj, Robot* robot) {
    ...
    if (challenge == "RobotS" && robot->getRobotType() == S) exp += 50;
    if (challenge == "RobotC" && robot->getRobotType() == C) exp += 50;
    if (challenge == "RobotSW" && robot->getRobotType() == SW) exp += 50;
    exp -= 50;
    if (exp < 0) exp = 0;
    watson->setExp(exp); 
    switch (robot->getRobotType()) {
...
void StudyPinkProgram ::run(bool verbose) {
    ...
    for (int istep = 0; istep < config->num_steps; ++istep) {
        ...
        if (watson->getCurrentPosition() == sherlock->getCurrentPosition()) {
            bool flag = 1;
            if(watson_bag->getsize() >= 15 || sherlock_bag->getsize() >= 13) flag = 0;
            if(flag){
            for (int i = 0; i < 15; i++) {
                if (watson_bag->getsize() >= 15) {
                    flag = 0;
                    break;
                } else {
